                ...
                do
                    ...
                    do
                        ...
                        do
                            for nz in ${NZ[@]}
                            do
                                NAME="UNCGAN_Noise"_"${M[0]}"_"$TYPE"
                                LOGGINGNAME=_"UNCGAN_Noise"_"${M[0]}"_+"Glr"_"$OPT_G"+"Dlr"_"$OPT_D"
                                jq --arg a "$NAME" '.name = $a' ./config/config_UNCGAN_NoiseM_HParamTune.json | sponge ./config/config_UNCGAN_NoiseM_HParamTune.json
                                jq --arg a "$LOGGINGNAME" '.run_name = $a' ./config/config_UNCGAN_NoiseM_HParamTune.json | sponge ./config/config_UNCGAN_NoiseM_HParamTune.json
                                ...
                                -c ./config/config_UNCGAN_NoiseM_HParamTune.json\
                                # --opt_G "$OPT_G"\
                                # --opt_D "$OPT_D"\
                                # --ngf "$ngf"\
                                # --ndf "$ndf"\
