...
class CasUNet_3head_Segmented(nn.Module):
	def __init__(self, n_unet, io_channels, bilinear=True):
		super(CasUNet_3head_Segmented, self).__init__()
		self.n_unet = n_unet
		self.io_channels = io_channels
		self.bilinear = bilinear
		####
		...
		for i in range(self.n_unet):
			if i != self.n_unet-1:
				self.unet_list.append(UNet(self.io_channels, self.io_channels*3, self.bilinear))
			else:
				self.unet_list.append(UNet_3head(self.io_channels, self.io_channels*3, self.bilinear))
	def forward(self, x):
		y = x
		for i in range(self.n_unet):
			if i==0:
...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        if self.hybrid:
            self.hybrid_model = ResNetV2(block_units=config.resnet.num_layers, width_factor=config.resnet.width_factor, begin_channel=in_channels)
...
class Transformer(nn.Module):
    def __init__(self, config, img_size, vis,in_channel):
        super(Transformer, self).__init__()
        self.embeddings = Embeddings(config, img_size=img_size,in_channels=in_channel)
...
class Generation3Head(nn.Sequential):

    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.out_mean = OutConv(in_channels, out_channels)
        self.out_alpha = nn.Sequential(
            OutConv(in_channels, 128),
            OutConv(128, 1),
            nn.ReLU()
        )
        self.out_beta = nn.Sequential(
...
class VisionTransformer(nn.Module):
    def __init__(self, vit_name, img_size=224, out_channels=3, in_channel=3,zero_head=False, vis=False):
        super(VisionTransformer, self).__init__()
        config = CONFIGS_ViT[vit_name]
        self.out_channels = out_channels
        self.zero_head = zero_head
        self.classifier = config.classifier
        self.transformer = Transformer(config, img_size, vis,in_channel)
        ...
        self.generation3Head = Generation3Head(config['decoder_channels'][-1],out_channels)
        self.config = config
    ...
    def forward(self, x):
        ...
        y_mean, y_alpha, y_beta = self.generation3Head(x)
        return y_mean, y_alpha, y_beta
