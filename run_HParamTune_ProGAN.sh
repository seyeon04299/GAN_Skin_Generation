        ...
        do
            ...
            do
                ...
                do
                    ...
                    do
                        NAME="ProGAN"_"$TYPE"
                        LOGGINGNAME=_"ProGAN"_"$TYPE"+"Glr"_"$OPT_G"+"Dlr"_"$OPT_D"+"Gngf"_"$ngf"+"Dndf"_"$ndf"+"NZ"_"$nz"
                        jq --arg a "$NAME" '.name = $a' ./config/config_ProGAN_HParamTune.json | sponge ./config/config_ProGAN_HParamTune.json
                        jq --arg a "$LOGGINGNAME" '.run_name = $a' ./config/config_ProGAN_HParamTune.json | sponge ./config/config_ProGAN_HParamTune.json
                        jq --arg a "$TYPE" '.data_loader.args.cell_type = $a' ./config/config_ProGAN_HParamTune.json | sponge ./config/config_ProGAN_HParamTune.json
                        # jq '.' ./config/config_HParamTune_DCGAN.json
                        # echo $LOGGINGNAME
                        python3 train.py --device 0,1\
                        -c ./config/config_ProGAN_HParamTune.json\
                        --opt_G "$OPT_G"\
                        --opt_D "$OPT_D"\
                        --ngf "$ngf"\
                        --ndf "$ndf"\
